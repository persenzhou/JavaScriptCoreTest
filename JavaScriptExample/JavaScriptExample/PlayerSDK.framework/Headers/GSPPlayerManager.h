...
typedef NS_ENUM(NSInteger, GSPayType) {
...
/**
 ...
 @param money 打赏金额，单位分；最小：1，最大：0x7FFFFFFF
 @param comment 打赏留言
 @param payType 付款方式
 @param expireTime 打赏订单有效时间，单位秒；在订单生成后有效期内没有成功支付，则认为订单失效；默认900，即15分钟；最大86400（即1天），最小1
 @param extra 额外参数，目前未使用
 */
- (void)rewardMoney:(unsigned)money comment:(NSString*)comment payType:(GSPayType)payType expireTime:(NSUInteger)expireTime extra:(NSString*)extra;
